...
//---------------------------------------------------------------------
    ...
    public void a_request_is_prepared_to_get_update_employee() {
        request = given().
                header(APIConstants.HEADER_CONTENT_TYPE_KEY,
                        APIConstants.HEADER_CONTENT_TYPE_VALUE).
                header(APIConstants.HEADER_AUTHORIZATION_KEY, token).
    ...
    @Then("the status code for the request should be {int}")
    public void the_status_code_for_the_request_should_be(Integer statusCode) {
        response.then().assertThat().statusCode(statusCode);
        response.prettyPrint();
    }
